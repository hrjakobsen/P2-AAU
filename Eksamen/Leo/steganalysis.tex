% -*- root: ../Presentation.tex -*-
\section{Steganalysis}
\begin{frame}{Steganalysis}{}
	Steganalysis techniques used:
	\begin{itemize}
		\item Colour histogram
		\item LSB enhancing
		\item Sammenligning med original bilede
		\item DCT coefficients histogram
		\item Steganographic signatures/patterns
	\end{itemize}
\end{frame}

\subsection{Colour histogram} %%% TODO: Add info about how many forces were made
\begin{frame}{Colour histogram}{}
\begin{figure}
\centering     %%% not \center
\subfigure[Original]{\label{fig:Room}\includegraphics[width=.4\textwidth]{figures/roomOriginal.jpg}}
\end{figure}
\begin{figure}
\centering
\subfigure[Original 1 char]{\label{fig:Room1char}\includegraphics[width=.4\textwidth]{figures/room1char.png}}
\subfigure[Stego Intro 2896 chars]{\label{fig:RoomIntro}\includegraphics[width=.4\textwidth]{figures/roomEncodedIntroToSteg.png}}
\caption{Sammenligning af colour histogram}
\end{figure}
\note{Stego intro is 2896 bytes or 2.896 kilobyte}
\end{frame}

\subsection{LSB enhancing}
\begin{frame}{LSB enhancing}
	LSB enhancing
	\begin{itemize}
		\item fjerne alle 7 højtstående bits
		\item Alle bytes har nu værdien 0 eller 1
		\item Alle 0 bytes forbliver 0, imens alle 1 bytes bilver 255
	\end{itemize}
\end{frame}

\begin{frame}{LSB enhancing}
\begin{figure}
\centering
\subfigure[Original]{\label{fig:Room}\includegraphics[width=.45\textwidth]{figures/roomOriginal.jpg}}
\subfigure[Original LSB enhanced]{\label{fig:Room}\includegraphics[width=.45\textwidth]{figures/roomOriLSB.jpg}}
\end{figure}
\end{frame}

\begin{frame}{LSB enhancing}
\begin{figure}
\centering
\subfigure[Original LSB enhanced]{\label{fig:Room}\includegraphics[width=.45\textwidth]{figures/roomOriLSB.jpg}}
\subfigure[Stego Intro LSB enhanced]{\label{fig:Room}\includegraphics[width=.45\textwidth]{figures/roomIntroLSB.jpg}}
\end{figure}
\note{Stego intro is 2896 bytes or 2.896 kilobyte}
\end{frame}

\subsection{Sammenligning med original bilede}
\begin{frame}{Sammenligning med original bilede}
\begin{figure}
\centering
\subfigure[Original]{\label{fig:a}\includegraphics[width=.3\textwidth]{figures/roomOriginal.jpg}}
\end{figure}
\begin{figure}
\centering
\subfigure[Stego Intro vs original]{\label{fig:b}\includegraphics[width=.4\textwidth]{figures/introToStegovs1chardiff.jpg}}
\subfigure[Lorem Ipsum 1. paragraph vs original]{\label{fig:a}\includegraphics[width=.4\textwidth]{figures/loremIpsum1paragraphvs1chardiff.png}}
\caption{Sammenlligning af bileder}
\end{figure}
\note{
Lorem Ipsum 1. paragraph is 500 bytes 
Stego intro is 2896 bytes}
\end{frame}