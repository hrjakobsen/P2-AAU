...
Encoding half that, 4480 bytes, took around three minutes, so extrapolating from that, it would take roughly 50 hours to encode 40,000 bytes.

This made a direct comparison using realistic real-world images and messages impossible. 
Instead we could use an image small enough to fill it out with data using both our programmes.
