...
\label{alg1}
\begin{algorithmic}
\REQUIRE True-color bitmap images $c = \{c^1,c^2\ldots c^{WH}\}$ with size $W \times H$ where $c^{xW+y}$ is the pixel at location $(x,y)$ and $m  = \{m^1,m^2\ldots m^{(WH)/4}\}$ with size $W/2 \times H/2$ where $m^{xW/2+y}$ is the pixel at location $(x,y)$
\ENSURE True-color bitmap image $s  = \{s^1,s^2\ldots s^{WH}\}$ of size $W \times H$ and where $s^{xW+y}$ is the pixel at location $(x,y)$ and where $m$ is embedded in $c$
