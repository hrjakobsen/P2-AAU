...
Because of the quantization process, there are now a lot of zeroes in the lower-right corner of the 8x8 block of pixels. This feature can be utilized to compress the image data, but first the table must be ordered so that the zeroes are placed next to each other. 
...
After the run-length encoding, the pairs must be written to the file, but before that can happen, the JPEG standards define even more compression of our data. Instead of storing the actual values from the vector, a category of the number, along with a bit representation is stored. 
