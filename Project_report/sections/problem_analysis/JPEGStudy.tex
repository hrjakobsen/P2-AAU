...
Before sampling, the image must be split into smaller blocks, called the MCUs\index{MCU} or Minimum Coded Units. The default size of an MCU is 8x8 pixels, but it changes when sampling is used. the most commonly used sampling is 4:2:0 where the chroma channels where downscaled by 50\% in both directions. 
