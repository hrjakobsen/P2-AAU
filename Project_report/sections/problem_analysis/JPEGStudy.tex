...
The JPEG specification allows 15[SOURCE NEEDED, JEG ER IKKE SIKKER] quantization tables to be used, while JFIF limits it even further to 2 tables. 
...
Because of the quantization process, there is now a lot of zeroes in the lower-right corner of the 8x8 block of pixels. This feature can be utilized to compress the image data, but first the table must be ordered so that the zeroes are placed next to each other. 
...
The first entry in the vector is called the DC coefficient, and will be encoded differently from the other 63. To encode the last 63 entries, run-length encoding (RLE) is used. Each value different from zero in the vector is represented as $(\text{Number of zeroes before this entry}, \text{Entry})$. Because of the many consecutive zeroes in the data, this allows a very compact encoding of the numbers. If at one point in the vector, only zeroes remains in the vector, we end the block with the EOB value (0,0). 

The DC coefficient is often large numbers, so it would be a waste of bits to write it in full to the file each time. Instead only the difference from the last DC coefficient is stored, so that smaller numbers needs to be encoded. So when reading a JPEG file, a decoder must keep track of the last DC coefficient. The first coefficient encoded, of course is the whole number.

There are some restrictions to this encoding, as only a nibble is used to represent the number of preceding zeroes, the maximum value we can denote is $F_{16}=15_{10}$. If there is more than 15 consecutive zeroes, we can encode 16 zeroes as (15,0), and by using multiple of these values, denote any number of consecutive zeroes. 

\subsection*{Huffman coding}
After the run-length encoding, the pairs must be written to the file, but before that can happen, the JPEG standards defines even more compression of our data. Instead of storing the actual values from the vector, a category of the number, along with a bit representation is stored. 
...
The vector entry of $(0,6)$ would become $(0,(3,110))$ when encoded as shown above. The last step is now to Huffman code these pairs of numbers. In a 8x8 block of pixels, there may be a lot of $(0,6)$, so instead of having to spends two bytes every time this pair appears, we use Huffman Tables. 

A Huffman Table is a table where you can look up pair of numbers, and get a bit representation which corresponds to the pair of numbers. To complete the example with the entry $(0,6)$ which was encoded to $(0,3,110)$, $(0,3)$ is looked up in the Huffman Table and it can be seen that it has a category of $3$, and a bit representation of $100$. 
...
There are no universal Huffman tables, and JPEG allows an encoder to define its own. They do however provide some Huffman tables which has shown good results with a variety of images \citep[p. 153]{JPEGStandard}. 
...
The JFIF standard allows a JFIF file to use two quantization tables, which means that some channels have to share quantization tables. Because of this, each quantization tables gets an ID, so that each component can point to a quantization table. 
...
The Define Huffman Table segment describes how values are encoded in the the image data. The first part of the segment defines whether the table is used for DC or AC components. Next an ID is defined, so that each channel can be linked to a certain Huffman Table.

The next 16 bytes defines how many codes of a certain length is encoded. The first byte describes how many codes of length one is encoded, the next describes how many codes of length two is encoded and so on, until all 16 bytes are used. 
...
toptitle=0.5mm,center title, bottomtitle=0.5mm,title=Component Segment]{
	 	\begin{tabular}{|l|l|l|}
			...
			4 bits byte & Horizontal sampling \\ \hline
			4 bits byte & Verticalnyt hundikat billed/figur sampling \\ \hline
...
In the beginning of this section it was explained that the a marker with the name FF00 does not exist. The reason for this is, that if the scan data must encode the byte $1111 1111_2=\text{FF}_{16}$ it cannot do so without it being read as a marker. Therefore the JPEG specification reads that a two consecutive bytes $\text{FF}00$ must be read as $\text{FF}$ and not as a marker. 
