...
\label{alg1}
\begin{algorithmic}
\REQUIRE True-color images $v = \{v^1,v^2\ldots v^{NM}\}$ with size $M \times N$ and $c  = \{c^1,c^2\ldots c^{(NM)/4}\}$ where $c^{xN/2+y}$ is the pixel at location $(x,y)$
\ENSURE True-color image $h  = \{h^1,h^2\ldots h^{NM}\}$ of size $M \times N$ and where $h^{xN+y}$ is the pixel at location $(x,y)$ and where $c$ is embedded in $v$
