...
\label{alg1}
\begin{algorithmic}
\REQUIRE True-color images $c = \{c^1,c^2\ldots c^{WH}\}$ with size $H \times W$ and $m  = \{m^1,m^2\ldots m^{(WH)/4}\}$ where $m^{xW/2+y}$ is the pixel at location $(x,y)$
\ENSURE True-color image $s  = \{s^1,s^2\ldots s^{WH}\}$ of size $H \times W$ and where $s^{xW+y}$ is the pixel at location $(x,y)$ and where $m$ is embedded in $c$
