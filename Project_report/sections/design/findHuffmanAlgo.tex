...
\STATE{current code length $codeL := 0$}
...
\ENDFOR
	...
	\FOR {$i:=1$ \TO $i = length$}
		\WHILE{$e = 0$ \AND $cL < 17$}
			...
			\STATE{$e :=  AoL_{cL}$}
		\ENDWHILE
		\STATE{Create new Huffman element from the next read byte in the stream, with the same code as $code$ and with the length $AoL_{cL}$}
