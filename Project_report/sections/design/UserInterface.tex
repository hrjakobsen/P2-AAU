...
The figure as shown in \ref{fig:LSBForm} is what our UI looked like for our LSB experiment. We are taking the main ideas from this, where we show the user the images they have selected and the final product. 
\begin{figure}
	\centering
...
Something that we came across when creating the UI, was the programme's inability to save the custom settings. This would be  a useful tool, as quite a number of the custom components are quite time consuming to fill out, especially when having to type in each value into both the Huffman tables and quantization table forms.
