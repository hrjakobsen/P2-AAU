...
Some of these processes such as DCT are simply mathematical formulas performed on the data, and will not change from one image to another, but other processes such as Huffman coding and quantization can yield very different result, depending on what Huffman- or quantization tables were used. 
...
Because the Huffman- and quantization tables has such a tremendous impact on the resulting JPEG image, it would make sense to have the user able to change these to their liking. 
Because of this we will both the quantization tables and the Huffman tables will be created as classes of their own, and because of this, changing the tables in the encoder will become as easy as changing a property.
